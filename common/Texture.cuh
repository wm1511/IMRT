...
public:
	...
	__host__ __device__ virtual void color(float3& color, float2 uv) const = 0;
...
public:
	...
	__host__ __device__ void color(float3& color, const float2) const override
	{
		color = albedo_;
	}
...
public:
	...
	__host__ __device__ void color(float3& color, const float2 uv) const override
	{
		...
		color = make_float3(data_[texel_index], data_[texel_index + 1], data_[texel_index + 2]);
	}
...
public:
	...
	__host__ __device__ void color(float3& color, const float2 uv) const override
	{
		color = sin(inverse_size_ * uv.x) * sin(inverse_size_ * uv.y) < 0.0f ? albedo_a_ : albedo_b_;
	}
